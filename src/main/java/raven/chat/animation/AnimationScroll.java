...
public class AnimationScroll
{
	...
	{
		this.animator = new Animator(750);
	...
	public void scrollHorizontal(JScrollPane scroll, int values)
	{
		...
		target = new PropertySetter(scroll.getHorizontalScrollBar(), "value",
				scroll.getHorizontalScrollBar().getValue(), values);
	...
	public void scrollVertical(JScrollPane scroll, int values)
	{
		stop();
		animator.removeTarget(target);
		target = new PropertySetter(scroll.getVerticalScrollBar(), "value", scroll.getVerticalScrollBar().getValue(),
				values);
		animator.addTarget(target);
		animator.start();
	}
